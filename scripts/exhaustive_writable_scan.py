...
def build_specs(include_fr: bool) -> Dict[str, TargetSpec]:
    ...
    for prefix in PREFIXES:
        for area, ranges in BASE_BIT_RANGES.items():
            ...
            specs[name] = TargetSpec(
                name, "pref-bit", ranges, 1, _pref_bit_writer(prefix, area)
            )
        for area, ranges in BASE_WORD_RANGES.items():
            name = f"{prefix}-{area}"
            specs[name] = TargetSpec(
                name, "pref-word", ranges, 0xFFFF, _pref_word_writer(prefix, area)
            )
    ...
    for area, ranges in EXT_WORD_RANGES.items():
        if area == "FR" and not include_fr:
            continue
        specs[area] = TargetSpec(
            area, "ext-word", ranges, 0xFFFF, _ext_word_writer(area)
        )
    return specs
...
def _iter_targets(
    ...
    for name in requested:
        ...
        if key not in specs:
            raise SystemExit(
                f"Unknown target '{name}'. Use --targets all or explicit names like D,P1-D,U,GX."
            )
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="Exhaustive writable-range scan for every address in selected device families"
    )
    ...
    p.add_argument(
        "--targets",
        default="all",
        help="comma-separated target families, e.g. D,P1-D,U or all",
    )
    p.add_argument("--include-fr", action="store_true")
    p.add_argument(
        "--step", type=int, default=1, help="coarse scan step size (default: 1)"
    )
    p.add_argument(
        "--reverse",
        action="store_true",
        help="scan each range from high address to low address",
    )
    p.add_argument(
        "--refine-boundary",
        action="store_true",
            ...
            for start, end in spec.ranges:
                ...
                if (
                    ...
                    tail_index = (
                        ...
                        else (end if args.reverse else start)
                    )
