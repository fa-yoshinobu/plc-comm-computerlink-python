...
def _describe_last_result(plc: ToyopucDeviceClient) -> str:
    ...
    if final.rc == 0x10:
        ...
        return (
            f"last_rx={rx_hex} inner_rc=0x10 response_error={_format_error_code(code)}"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--out-of-range-word-index", type=parse_int_auto, default=0x3000
    )
    ...
    try:
        ...
        _emit(
            f"forbidden_write_device = {args.forbidden_write_device or '<skip>'}", log_f
        )
        _emit(f"out_of_range_word_index = 0x{args.out_of_range_word_index:04X}", log_f)
        if args.out_of_range_pc10_addr32 is not None:
            _emit(
                f"out_of_range_pc10_addr32 = 0x{args.out_of_range_pc10_addr32:08X}",
                log_f,
            )
        ...
        with ToyopucDeviceClient(
            ...
            def run(name: str, fn) -> None:
                ...
                if hasattr(result, "rc") and getattr(result, "rc") == 0x10:
                    ...
                    return
                _emit(
                    f"{name}: FAIL unexpected success; {_describe_last_result(plc)}",
                    log_f,
                )

            run(
                "missing-station cpu-status",
                lambda: plc.relay_read_cpu_status(missing_hops),
            )
            run("broken-path clock-read", lambda: plc.relay_read_clock(broken_hops))
            run(
                "out-of-range basic-word read",
                lambda: plc.send_via_relay(
                    args.hops, build_word_read(0x1000 + args.out_of_range_word_index, 1)
                ),
            )
            ...
            if args.out_of_range_pc10_addr32 is not None:
                run(
                    ...
                    lambda: plc.send_via_relay(
                        args.hops,
                        build_pc10_block_read(args.out_of_range_pc10_addr32, 2),
                    ),
                )
            ...
            if args.forbidden_write_device:
                run(
                    ...
                    lambda: plc.relay_write(
                        args.hops,
                        args.forbidden_write_device,
                        args.forbidden_write_value,
                    ),
                )
            else:
                _emit(
                    "forbidden-write: SKIP (use --forbidden-write-device to test a known protected address)",
                    log_f,
                )
