...
def _describe_last_result(plc: ToyopucDeviceClient) -> str:
    ...
    if final is None:
        ...
        return f"last_rx={rx_hex} relay_ack=0x{ack:02X} relay_error={_format_error_code(relay_code)}"

    if final.rc == 0x10:
        code = final.data[-1] if final.data else final.cmd
        return f"last_rx={rx_hex} inner_rc=0x10 response_error={_format_error_code(code)}"
