...
def _print_result(result: ProbeResult, log_f) -> None:
    _print_and_log(
        f"{result.label}: {'OK' if result.ok else 'ERR'} {result.detail}", log_f
    )
...
def probe_pc10_fr(
    plc: ToyopucClient, index: int, *, label_prefix: str = "pc10-c2"
) -> ProbeResult:
    addr32 = encode_fr_word_addr32(index)
    ex_no = fr_block_ex_no(index)
    label = (
        f"{label_prefix} index=0x{index:06X} ex_no=0x{ex_no:02X} addr32=0x{addr32:08X}"
    )
    try:
        ...
        with ToyopucClient(
            ...
            for index in indexes:
                ...
                for ex_no in register_exnos:
                    ...
                    after = probe_pc10_fr(
                        plc, index, label_prefix=f"after-ca=0x{ex_no & 0xFF:02X}"
                    )
            ...
            _print_and_log(
                f"SUMMARY: {success_count}/{total_count} probes succeeded", log_f
            )
