...
def parse_hex_bytes(text: str) -> bytes:
    ...
    if len(cleaned) % 2 != 0:
        raise argparse.ArgumentTypeError(
            "hex byte string must contain an even number of hex digits"
        )
    try:
...
def build_inner_payload(args) -> bytes:
    ...
    if args.inner == "clock-write":
        if args.clock_value is None:
            raise SystemExit(
                "--clock-value is required when --inner clock-write is used"
            )
...
def print_decoded_inner(args, inner_resp, printer: Callable[[str], None]) -> None:
    if args.inner == "cpu-status":
        ...
        printer(
            f"INNER CPU_STATUS Under writing flash register = {status.under_writing_flash_register}"
        )
        printer(
            f"INNER CPU_STATUS Abnormal write flash register = {status.abnormal_write_flash_register}"
        )
        return
    if args.inner == "cpu-status-a0":
        ...
        printer(
            f"INNER CPU_STATUS_A0 Under writing flash register = {status.under_writing_flash_register}"
        )
        printer(
            f"INNER CPU_STATUS_A0 Abnormal write flash register = {status.abnormal_write_flash_register}"
        )
        return
    if args.inner == "clock-read":
    ...
    if args.inner == "word-read":
        ...
        printer(
            "INNER WORD_READ values = " + ", ".join(f"0x{value:04X}" for value in words)
        )
        return
    if args.inner == "word-write":
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Real-hardware relay command (`CMD=60`) test helper."
    )
    ...
    p.add_argument(
        "--device", default="D0000", help="word device for --inner word-read"
    )
    p.add_argument(
        "--count",
        type=parse_int_auto,
        default=1,
        help="word count for --inner word-read",
    )
    p.add_argument(
        "--value",
        type=parse_int_auto,
        default=0x1234,
        help="word value for --inner word-write",
    )
    p.add_argument(
        "--clock-value",
        type=parse_datetime_iso,
        default=None,
        help="ISO datetime for --inner clock-write",
    )
    p.add_argument(
        "--raw-inner",
        type=parse_hex_bytes,
        default=b"",
        help="full inner frame bytes for --inner raw",
    )
...
def main() -> int:
    ...
    try:
        ...
        log(
            "HOPS = "
            + ", ".join(format_hop(link, station) for link, station in args.hops)
        )
        ...
        if resp.cmd != 0x60:
            raise ToyopucProtocolError(
                f"Unexpected outer CMD in relay response: 0x{resp.cmd:02X}"
            )
