...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Read or write/commit an FR word and print A0 status."
    )
    ...
    p.add_argument(
        "--target", default="FR000000", help="FR word device such as FR000000"
    )
    p.add_argument(
        "--value", type=parse_int_auto, default=0x1234, help="word value for write mode"
    )
    p.add_argument("--commit-timeout", type=float, default=30.0)
    p.add_argument("--poll-interval", type=float, default=0.2)
    p.add_argument(
        "--try-a0", action="store_true", help="try reading A0 status after read/write"
    )
    return p
...
def print_a0(prefix: str, status) -> None:
    ...
    print(
        f"{prefix} under_writing_flash_register = {status.under_writing_flash_register}"
    )
    print(
        f"{prefix} abnormal_write_flash_register = {status.abnormal_write_flash_register}"
    )
