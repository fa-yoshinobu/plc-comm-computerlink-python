...
def _verify_bit_sequence(
    ...
    for value in (0, 1):
        ...
        if read_back == value:
            ...
            if log_f:
                log_f.write(
                    f"{mismatch_prefix} exp={value} got={read_back} (tolerated)\n"
                )
        elif log_f:
...
def _decode_ext_multi_read_data(
    data: bytes, bit_count: int, byte_count: int, word_count: int
):
    ...
    words_out = [
        words_raw[i] | (words_raw[i + 1] << 8) for i in range(0, len(words_raw), 2)
    ]
    ...
    for idx in indices:
        try:
            ...
            o, t = _verify_bit_sequence(
                ...
                log_f,
                tolerated_label=f"BIT:{area}"
                if _is_tolerated_area("bit", area)
                else None,
            )
            ok += o
            total += t
    ...
    for idx in indices:
        try:
            ...
            o, t = _verify_word_sequence(
                ...
                rng,
                log_f,
                tolerated_label=f"WORD:{area}"
                if _is_tolerated_area("word", area)
                else None,
            )
            ok += o
            total += t
...
def run_bit_area_ranges(
    ...
    for start, end in ranges:
        ...
        o, t = _test_bit_indices(
            plc, area, indices, rng, log_f, skip_errors=skip_errors
        )
        ok += o
        total += t
    return ok, total
...
def run_max_block_lengths(
    ...
    run(
        ...
        lambda: _verify_word_block_sequence(
            lambda values: plc.write_words(
                encode_word_address(parse_address("D0000", "word")), values
            ),
            lambda: plc.read_words(
                encode_word_address(parse_address("D0000", "word")), 0x200
            ),
            lambda suffix: _log_frames(
                log_f, plc, f"[BLOCK WORD] D0000 x0200 {suffix}"
            ),
    ...
    run(
        ...
        lambda: _verify_byte_block_sequence(
            lambda values: plc.write_bytes(
                encode_byte_address(parse_address("D0000L", "byte")), values
            ),
            lambda: plc.read_bytes(
                encode_byte_address(parse_address("D0000L", "byte")), 0x400
            ),
            lambda suffix: _log_frames(
                log_f, plc, f"[BLOCK BYTE] D0000L x0400 {suffix}"
            ),
    ...
    run(
        ...
        lambda: _verify_word_block_sequence(
            ...
            lambda suffix: _log_frames(
                log_f, plc, f"[BLOCK EXT WORD] U00000 x0200 {suffix}"
            ),
    ...
    run(
        ...
        lambda: _verify_byte_block_sequence(
            ...
            lambda suffix: _log_frames(
                log_f, plc, f"[BLOCK EXT BYTE] U00000 x0400 {suffix}"
            ),
    ...
    run(
        ...
        lambda: _verify_word_block_sequence(
            lambda values: plc.pc10_block_write(
                _pc10_u_word_addr32(0x08000), b"".join(_pack_u16_le(v) for v in values)
            ),
            lambda: _unpack_words_block(
                plc.pc10_block_read(_pc10_u_word_addr32(0x08000), pc10_word_count * 2)
            ),
            lambda suffix: _log_frames(
                log_f, plc, f"[BLOCK PC10 WORD] U08000 x{pc10_word_count:04X} {suffix}"
            ),
    ...
    run(
        ...
        lambda: _verify_word_block_sequence(
            lambda values: plc.pc10_block_write(
                _pc10_eb_word_addr32(0x00000), b"".join(_pack_u16_le(v) for v in values)
            ),
            lambda: _unpack_words_block(
                plc.pc10_block_read(_pc10_eb_word_addr32(0x00000), pc10_word_count * 2)
            ),
            lambda suffix: _log_frames(
                log_f, plc, f"[BLOCK PC10 WORD] EB00000 x{pc10_word_count:04X} {suffix}"
            ),
...
def _verify_ext_multi_pair_case(
    ...
    if (
        bits1 != expected1_bits
        or bytes1 != expected1_bytes
        or words1 != expected1_words
    ):
        if log_f:
            log_f.write(
                f"[EXT MULTI] MISMATCH {label} write1 bit={bits1} byte={bytes1} word={words1}\n"
    ...
    if (
        bits2 != expected2_bits
        or bytes2 != expected2_bytes
        or words2 != expected2_words
    ):
        if log_f:
            log_f.write(
                f"[EXT MULTI] MISMATCH {label} write2 bit={bits2} byte={bytes2} word={words2}\n"
...
def run_ext_multi_mixed(
    ...
    cases.append(
        (
            ...
            (
                encode_ext_no_address("U", 0x0000, "byte").no,
                encode_ext_no_address("U", 0x0000, "byte").addr,
            ),
            (
    ...
    cases.append(
        (
            ...
            (
                encode_ext_no_address("GXY", 0x0001, "byte").no,
                encode_ext_no_address("GXY", 0x0001, "byte").addr,
            ),
            (
    ...
    split_cases: List[
        ...
        (
            ...
            _ext_bit_point("GX", 0x0000),
            (
                encode_ext_no_address("GXY", 0x0000, "byte").no,
                encode_ext_no_address("GXY", 0x0000, "byte").addr,
            ),
            None,
        ),
        (
...
def run_boundary_values(
    ...
    run(
        ...
        lambda: (lambda a, b: (a[0] + b[0], a[1] + b[1]))(
            _test_bit_indices(
                plc, "L", [0x07FE, 0x07FF], rng, log_f, skip_errors=skip_errors
            ),
    ...
    run(
        ...
        lambda: (lambda a, b: (a[0] + b[0], a[1] + b[1]))(
            _test_bit_indices(
                plc, "M", [0x07FE, 0x07FF], rng, log_f, skip_errors=skip_errors
            ),
            _test_pc10_bit_area_ranges_with_builder(
                plc,
                "M",
        ...
        for idx in indices:
            try:
                ...
                o, t = _verify_bit_sequence(
                    ...
                    log_f,
                    tolerated_label=f"BIT:{label}"
                    if _is_tolerated_area("bit", label)
                    else None,
                )
                ok += o
                total += t
        ...
        for idx in indices:
            try:
                ...
                o, t = _verify_word_sequence(
                    ...
                    rng,
                    log_f,
                    tolerated_label=f"WORD:{label}"
                    if _is_tolerated_area("word", label)
                    else None,
                )
                ok += o
                total += t
...
def _ext_multi_write_bit(
    plc: ToyopucClient, no: int, bit_no: int, addr: int, value: int
) -> None:
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="local UDP source port (default: auto)",
    )
    ...
    p.add_argument(
        "--include-io", action="store_true", help="include X/Y/L (can affect hardware)"
    )
    p.add_argument(
        "--include-special",
        action="store_true",
        help="include S/N/R (may be read-only)",
    )
    p.add_argument(
        "--include-extended", action="store_true", help="include U/EB extended areas"
    )
    ...
    if args.pc10g_full:
        ...
        if args.include_all or args.include_io:
            bit_areas.update(
                {"L": (0x000, 0x7FF), "X": (0x000, 0x7FF), "Y": (0x000, 0x7FF)}
            )
        ...
        if args.include_all or args.include_special:
            word_areas.update(
                {"S": (0x0000, 0x03FF), "N": (0x0000, 0x01FF), "R": (0x0000, 0x07FF)}
            )
    ...
    with ToyopucClient(
        ...
        if args.max_block_test:
            for label, ok, t in run_max_block_lengths(
                ...
                pc10_word_count=args.pc10_block_words,
            ):
        ...
        if args.pc10g_full:
            ...
            for area, ranges in ext_word_area_ranges.items():
                ...
                if area == "EB":
                    ...
                    label = _ranges_label(
                        "EB", [(0x00000, 0x3FFFF), (0x40000, 0x7FFFF)]
                    )
                    _print_result("[EXT WORD]", label, ok1 + ok2, t1 + t2, log_f)
                    total_ok += ok1 + ok2
                    total += t1 + t2
