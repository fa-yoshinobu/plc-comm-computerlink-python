...
def pick_indices_min_max(rng: random.Random, start: int, end: int, count: int) -> list[int]:
    span = end - start + 1
    if span <= 0:
        return []
    if span == 1:
...
def _verify_word_sequence(
    ...
    for label, value in (("write1", first_value), ("write2", first_value ^ 0xFFFF)):
        ...
        if read_back == value:
            ...
            ok += 1
            if log_f:
                log_f.write(f"{mismatch_prefix} exp=0x{value:04X} got=0x{read_back:04X} (tolerated)\n")
        elif log_f:
        ...
        try:
            ...
            o, t = _verify_bit_sequence(
                ...
                lambda suffix: _log_frames(
                    log_f,
                    plc,
                    f"[BIT] {area}{idx:04X} {suffix}",  # noqa: B023
                ),
        ...
        try:
            ...
            o, t = _verify_word_sequence(
                ...
                lambda suffix: _log_frames(
                    log_f,
                    plc,
                    f"[WORD] {area}{idx:04X} {suffix}",  # noqa: B023
                ),
        ...
        try:
            ...
            o, t = _verify_word_sequence(
                ...
                lambda suffix: _log_frames(
                    log_f,
                    plc,
                    f"[EXT WORD] {area} {idx:06X} {suffix}",  # noqa: B023
                ),
...
def run_ext_word_area_ranges(
    ...
    for start, end in ranges:
        indices = pick_indices_min_max(rng, start, end, count)
        o, t = _test_ext_word_indices(plc, area, indices, rng, log_f, skip_errors=skip_errors, encoder=encoder)
        ok += o
        total += t
    return ok, total
    ...
    for idx in pick_indices(rng, start, end, count):
        try:
            ...
            if read_back == value:
                ...
                log_f.write(f"[BYTE] MISMATCH {area}{idx:04X}L exp=0x{value:02X} got=0x{read_back:02X}\n")
        except ToyopucError as e:
            if not skip_errors:
                raise
            if log_f:
    ...
    for idx in pick_indices(rng, start, end, count):
        try:
            ...
            if read_back == value:
                ...
                log_f.write(f"[EXT BYTE] MISMATCH {area} {idx:06X} exp=0x{value:02X} got=0x{read_back:02X}\n")
        except (ToyopucError, ValueError) as e:
            if not skip_errors:
                raise
            if log_f:
...
def _verify_ext_multi_case(
    ...
    if bits1 != [0] or bytes1 != [byte_value1] or words1 != [word_value1]:
        if log_f:
            log_f.write(f"[EXT MULTI] MISMATCH {label} write1 bit={bits1} byte={bytes1} word={words1}\n")
        return 0, 1
    ...
    if bits2 != [1] or bytes2 != [byte_value2] or words2 != [word_value2]:
        if log_f:
            log_f.write(f"[EXT MULTI] MISMATCH {label} write2 bit={bits2} byte={bytes2} word={words2}\n")
        return 0, 1
    return 1, 1
...
def run_ext_multi_mixed(
    ...
    for label, bit_point, byte_point, word_point in cases:
        try:
            ok, total = _verify_ext_multi_case(plc, label, bit_point, byte_point, word_point, rng, log_f)
        except (ToyopucError, ValueError) as e:
            if not skip_errors:
                raise
            if log_f:
            ...
            try:
                ...
                o, t = _verify_bit_sequence(
                    ...
                    lambda suffix: _log_frames(
                        log_f,
                        plc,
                        f"[PC10 BIT] {area}{idx:04X} {suffix}",  # noqa: B023
                    ),
            ...
            try:
                ...
                o, t = _verify_bit_sequence(
                    ...
                    lambda suffix: _log_frames(
                        log_f,
                        plc,
                        f"[PC10 BIT] {label}{idx:04X} {suffix}",  # noqa: B023
                    ),
            ...
            try:
                ...
                o, t = _verify_bit_sequence(
                    ...
                    lambda suffix: _log_frames(
                        log_f,
                        plc,
                        f"[EXT BIT] {area}{idx:04X} {suffix}",  # noqa: B023
                    ),
            ...
            try:
                ...
                o, t = _verify_word_sequence(
                    ...
                    lambda suffix: _log_frames(
                        log_f,
                        plc,
                        f"[PC10 WORD] {label}{idx:05X} {suffix}",  # noqa: B023
                    ),
            ...
            try:
                ...
                o, t = _verify_bit_sequence(
                    ...
                    lambda suffix: _log_frames(
                        log_f,
                        plc,
                        f"[PREF BIT] {prefix}-{area}{idx:04X} {suffix}",  # noqa: B023
                    ),
            ...
            try:
                ...
                o, t = _verify_word_sequence(
                    ...
                    lambda suffix: _log_frames(
                        log_f,
                        plc,
                        f"[PREF WORD] {prefix}-{area}{idx:04X} {suffix}",  # noqa: B023
                    ),
...
def main() -> int:
    ...
    if args.pc10_block_words > 0x200:
        print(f"WARNING: --pc10-block-words={args.pc10_block_words:#x} exceeds the verified safe default 0x200.")
    ...
    with ToyopucClient(
        ...
        if args.ext_multi_test:
            for label, ok, t in run_ext_multi_mixed(plc, rng, log_f, skip_errors=args.skip_errors):
                _print_result("[EXT MULTI]", label, ok, t, log_f)
                total_ok += ok
                total += t
        if args.boundary_test:
            for label, ok, t in run_boundary_values(plc, rng, log_f, skip_errors=args.skip_errors):
