...
def main() -> int:
    ...
    host = (
        cli_args.host if cli_args.host is not None else _prompt("Host", "192.168.0.10")
    )
    port = cli_args.port if cli_args.port is not None else int(_prompt("Port", "0"))
    protocol = (
        cli_args.protocol
        if cli_args.protocol is not None
        else _prompt("Protocol (tcp/udp)", "tcp").lower()
    )
    timeout = (
        cli_args.timeout
        if cli_args.timeout is not None
        else float(_prompt("Timeout sec", "3.0"))
    )
    retries = (
        cli_args.retries
        if cli_args.retries is not None
        else int(_prompt("Retries", "0"))
    )
    ...
    log_path = (
        ...
        else _prompt("Log file (empty to skip)", "")
    )
        ...
        while True:
            ...
            try:
                ...
                if cmd == "wr":
                    ...
                    addrs = [
                        encode_word_address(parse_address(a, "word")) for a in args
                    ]
                    ...
                    addrs = [
                        encode_byte_address(parse_address(a, "byte")) for a in args
                    ]
                    ...
                    if area.upper() == "FR":
                        payload = build_pc10_block_read(
                            encode_fr_word_addr32(index), count * 2
                        )
                    else:
                        ext = encode_ext_no_address(area, index, "word")
                    ...
                    if area.upper() == "FR":
                        ...
                        payload = build_pc10_block_write(
                            encode_fr_word_addr32(index), data
                        )
                    else:
                        ext = encode_ext_no_address(area, index, "word")
                    ...
                    bit_points, byte_points, word_points, labels = (
                        _parse_ext_multi_read_specs(args)
                    )
                    ...
                    bit_points, byte_points, word_points, labels = (
                        _parse_ext_multi_write_specs(args)
                    )
                    payload = build_ext_multi_write(
                        bit_points, byte_points, word_points
                    )
                elif cmd == "pcbitr":
                    area = args[0]
                    index = _parse_ints([args[1]])[0]
                    ...
                    count = _parse_ints([args[2]])[0]
                    payload = build_pc10_block_read(
                        _pc10_word_addr(area, index), count * 2
                    )
                elif cmd == "pcww":
                    area = args[0]
                    index = _parse_ints([args[1]])[0]
