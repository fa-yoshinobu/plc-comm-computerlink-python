...
def _byte_to_bits_case(plc: ToyopucDeviceClient, byte_addr: str, rng: random.Random, log_f) -> tuple[int, int]:
    ...
    line = (
        f"{byte_addr} -> {bit_addr} write={_format_value(value, 2)} read_bits={read_bits} expected_bits={expected_bits}"
    )
    print(line)
    _write_log(log_f, line)
