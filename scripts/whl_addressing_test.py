...
def _paired_word_byte_case(
    ...
    _write_log(log_f, line)
    ok = int(
        read_word == value and read_low == expected_low and read_high == expected_high
    )
    return ok, 1
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="Packed word/byte access verification for bit-device families"
    )
    ...
    cases: list[tuple[str, Callable[[ToyopucDeviceClient], tuple[int, int]]]] = [
        ...
        (
            ...
            lambda plc: _paired_word_byte_case(
                plc, "M0010W", "M0010L", "M0010H", rng, log_f
            ),
        ),
        (
            "W/H/L word-byte relation prefixed",
            lambda plc: _paired_word_byte_case(
                plc, "P1-M0010W", "P1-M0010L", "P1-M0010H", rng, log_f
            ),
        ),
        (
            "W/H/L word-byte relation extended",
            lambda plc: _paired_word_byte_case(
                plc, "EX0010W", "EX0010L", "EX0010H", rng, log_f
            ),
        ),
        (
