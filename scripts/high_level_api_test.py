...
def _mixed_many_case(plc: ToyopucDeviceClient, hops: str, items: dict[str, int], log_f) -> tuple[int, int]:
...
def main() -> int:
    p = argparse.ArgumentParser(description="High-level API verification tool for ToyopucDeviceClient")
    ...
    p.add_argument(
        "--hops",
        default="",
        help='optional relay hops, for example "P1-L2:N2,P1-L2:N4"',
    )
    ...
    single_cases: list[tuple[str, Callable[[ToyopucDeviceClient], tuple[int, int]]]] = [
        ...
        (
            "single bit/extended",
            lambda plc: _single_bit_case(plc, args.hops, "EX0000", log_f),
        ),
        (
            "single word/extended",
            lambda plc: _single_word_case(plc, args.hops, "ES0000", rng, log_f),
        ),
    ]
    if args.include_pc10_word:
        single_cases.append(
            (
                "single word/pc10",
                lambda plc: _single_word_case(plc, args.hops, "U08000", rng, log_f),
            )
        )
    ...
    with ToyopucDeviceClient(
        ...
        for name, addr, values in sequence_cases:
            try:
                ok, total = _run_case(
                    name,
                    lambda addr=addr, values=values: _sequence_case(plc, args.hops, addr, values, log_f),
                    log_f,
                )
            except (ToyopucError, ValueError) as exc:
                error_cases += 1
        ...
        try:
            ok, total = _run_case(
                ...
                lambda: _mixed_many_case(plc, args.hops, mixed_items, log_f),
                log_f,
            )
        except (ToyopucError, ValueError) as exc:
            error_cases += 1
