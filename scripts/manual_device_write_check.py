                ...
                targets.append(
                    Target(
                        ...
                        write=cast(
                            ...
                            lambda plc, addr=addr: plc.write_bit(addr, True),
                        ),
                    )
                )
                ...
                for idx in (start, end):
                    ...
                    targets.append(
                        Target(
                            ...
                            write=cast(
                                Callable[[ToyopucClient], None],
                                lambda plc, no=no, bit_no=bit_no, addr=addr: (
                ...
                for idx in (start, end):
                    ...
                    targets.append(
                        Target(
                            ...
                            write=cast(
                                Callable[[ToyopucClient], None],
                                lambda plc, no=no, addr=addr: plc.write_ext_words(
                                    no, addr, [0xFFFF]
                                ),
                            ),
                        )
                ...
                if area == "U" and idx >= 0x08000:
                    ...
                    targets.append(
                        Target(
                            ...
                            write=cast(
                                Callable[[ToyopucClient], None],
                                lambda plc, addr32=addr32: plc.pc10_block_write(
                                    addr32, _pack_u16_le(0xFFFF)
                                ),
                            ),
                        )
                    )
                    ...
                    targets.append(
                        Target(
                            ...
                            write=cast(
                                Callable[[ToyopucClient], None],
                                lambda plc, no=ext.no, addr=ext.addr: (
...
def _prompt_step(index: int, total: int, target: Target) -> str:
    print()
    print(
        f"[{index}/{total}] next: [{target.kind}] {target.label} <= {target.value_text}"
    )
