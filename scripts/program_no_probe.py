...
def _build_phase_snapshot(
    ...
    *,
    ...
    return ProbeSnapshot(
        ...
        byte=None if base.byte is None else _next_distinct_8(base.byte, salt8, None if avoid is None else avoid.byte),
        word=None if base.word is None else _next_distinct_16(base.word, salt16, None if avoid is None else avoid.word),
    )
...
def _run_case(plc: ToyopucClient, case: ProbeCase, candidate_nos: Sequence[int], log_f) -> bool:
    ...
    if suspicious:
        _print_line(
            "verdict: SUSPICIOUS alias/match on candidate no=" + ",".join(f"0x{value:02X}" for value in suspicious),
            log_f,
        )
        return False
