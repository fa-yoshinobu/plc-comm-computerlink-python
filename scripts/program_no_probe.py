    ...
    *,
    ...
    for device in (bit, byte, word):
        ...
        expected_values.append(
            _require_field(device.no, f"{key}: resolved device missing number")
        )
    ...
    return ProbeCase(
        key=key, label=label, expected_no=expected_no, bit=bit, byte=byte, word=word
    )
...
def _read_snapshot(plc: ToyopucClient, case: ProbeCase, no: int) -> ProbeSnapshot:
    ...
    bits, bytes_out, words_out = _decode_ext_multi_read_data(
        data, bit_count, byte_count, word_count
    )
...
def _write_snapshot(
    plc: ToyopucClient, case: ProbeCase, no: int, snapshot: ProbeSnapshot
) -> None:
    bit_points = []
    byte_points = []
    word_points = []
...
def _build_phase_snapshot(
    ...
    *,
    ...
    return ProbeSnapshot(
        ...
        else _next_distinct_16(
            base.word, salt16, None if avoid is None else avoid.word
        ),
    )
...
def _cohere_snapshot(case: ProbeCase, snapshot: ProbeSnapshot) -> ProbeSnapshot:
    if not case.has_shared_bit_byte():
        return snapshot
    if (
        snapshot.bit is None
        or snapshot.byte is None
        or case.bit is None
        or case.bit.bit_no is None
    ):
        return snapshot
...
def _run_case(
    plc: ToyopucClient, case: ProbeCase, candidate_nos: Sequence[int], log_f
) -> bool:
    _print_line(f"=== {case.key} ({case.label}) ===", log_f)
    _print_line(f"expected no=0x{case.expected_no:02X}", log_f)
    _print_line(
        "warning: this probe temporarily writes the selected scratch points and restores them",
        log_f,
    )
    ...
    phase1 = _cohere_snapshot(
        case, _build_phase_snapshot(original, salt8=0xA5, salt16=0xA55A)
    )
    phase2 = _cohere_snapshot(
        case, _build_phase_snapshot(phase1, salt8=0x5A, salt16=0x5AA5, avoid=original)
    )
    suspicious: list[int] = []
    restored = False
        ...
        for phase_name, expected in (("phase1", phase1), ("phase2", phase2)):
            ...
            for candidate in candidate_nos:
                ...
                try:
                    ...
                    _print_line(
                        f"{phase_name} candidate no=0x{candidate:02X} ERR {exc}", log_f
                    )
                    continue
...
def main() -> int:
    ...
    selected_keys = [
        item.strip().lower() for item in args.cases.split(",") if item.strip()
    ]
        ...
        with ToyopucClient(
            ...
            for case in selected_cases:
                ...
                try:
                    ...
                    _print_line(
                        f"case {case.key}: ERROR {type(exc).__name__}: {exc}", log_f
                    )
                    ok = False
                if ok:
                    ok_count += 1
