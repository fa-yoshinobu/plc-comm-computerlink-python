...
def build_pattern_bytes(start_index: int, count: int, seed: int) -> bytes:
    return b"".join(
        ...
        for word in build_pattern_words(start_index, count, seed)
    )
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Write an FR range with a deterministic pattern, then commit at the end and verify by readback."
    )
    ...
    p.add_argument(
        "--start", type=parse_int_auto, required=True, help="FR start word index"
    )
    p.add_argument(
        "--end",
        type=parse_int_auto,
        required=True,
        help="FR end word index (inclusive)",
    )
    p.add_argument(
        "--chunk-words",
        type=parse_int_auto,
        default=0x200,
        help="words per C3/C2 transfer (default: 0x200)",
    )
    p.add_argument(
        "--progress-every",
        type=int,
        default=64,
        help="print progress every N successful chunks (0=disable)",
    )
    p.add_argument(
        "--seed",
        type=parse_int_auto,
        default=0xA500,
        help="pattern seed; word = (seed + index) & 0xFFFF",
    )
    p.add_argument(
        "--skip-commit",
        action="store_true",
        help="write and verify RAM work area only; do not issue CA",
    )
    p.add_argument(
        "--skip-verify", action="store_true", help="skip final readback verification"
    )
...
def main() -> int:
    ...
    total_chunks = sum(
        1 for _ in iter_fr_chunks(args.start, args.end, args.chunk_words)
    )
    ...
    try:
        with ToyopucClient(
            ...
            write_ok_chunks = 0
            for index, chunk_words in iter_fr_chunks(
                args.start, args.end, args.chunk_words
            ):
                ...
                try:
                    ...
                    if (
                        args.progress_every
                        and write_ok_chunks % args.progress_every == 0
                    ):
                        elapsed = max(time.monotonic() - start_time, 1e-9)
                        words_done = min(
                            write_ok_chunks * args.chunk_words, total_words
                        )
            ...
            if not write_errors and not args.skip_verify:
                ...
                for index, chunk_words in iter_fr_chunks(
                    ...
                    try:
                        actual = plc.pc10_block_read(
                            encode_fr_word_addr32(index), chunk_words * 2
                        )
                        verify.ok_chunks += 1
                        verify.crc32_actual = zlib.crc32(actual, verify.crc32_actual)
                        mismatch_words, first_mismatch = count_chunk_mismatches(
                            actual, expected, index
                        )
                        verify.mismatch_words += mismatch_words
                        if (
                            verify.first_mismatch_index is None
                            and first_mismatch is not None
                        ):
                            verify.first_mismatch_index = first_mismatch
                        if (
                            args.progress_every
                            and verify.ok_chunks % args.progress_every == 0
                        ):
                            elapsed = max(time.monotonic() - verify_start, 1e-9)
                            words_done = min(
                                verify.ok_chunks * args.chunk_words, total_words
                            )
                            progress = words_done / total_words * 100.0
                            rate = words_done / elapsed
                            line = (
    ...
    write_log(
        log_f,
        f"write_errors={len(write_errors)} verify_error_chunks={verify.error_chunks}",
    )
    write_log(
        log_f,
        f"verify_ok_chunks={verify.ok_chunks} mismatch_words={verify.mismatch_words}",
    )
    ...
    if verify.first_mismatch_index is None:
        ...
        write_log(
            log_f, f"first_mismatch={format_fr_word(verify.first_mismatch_index)}"
        )
    if write_errors:
    ...
    return (
        0
        if not write_errors and verify.error_chunks == 0 and verify.mismatch_words == 0
        else 1
    )
