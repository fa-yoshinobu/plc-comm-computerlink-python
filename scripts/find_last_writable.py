        ...
        if area in EXT_WORD_AREAS:
            ...
            return (
                Probe(
                    ...
                    write=cast(
                        Callable[[ToyopucClient, int], None],
                        lambda plc, value, no=ext.no, addr=ext.addr: plc.write_ext_words(no, addr, [value & 0xFFFF]),
                    ),
                ),
                0xFFFF,
            )
        ...
        if area in BASE_WORD_AREAS:
            ...
            return (
                Probe(
                    ...
                    write=cast(
                        Callable[[ToyopucClient, int], None],
                        lambda plc, value, no=no, addr=addr: plc.write_ext_words(no, addr, [value & 0xFFFF]),
                    ),
                ),
                0xFFFF,
            )
...
def main() -> int:
    ...
    if args.auto_pending:
        ...
        if not stop_text:
            ...
            stop_text = f"{area0}{0:0{width}X}" if prefix0 is None else f"{prefix0}-{area0}{0:0{width}X}"
