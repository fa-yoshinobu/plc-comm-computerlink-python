        ...
        if area in BASE_WORD_AREAS:
            ...
            return (
                Probe(
                    ...
                    write=cast(
                        Callable[[ToyopucClient, int], None],
                        lambda plc, value, addr=addr: plc.write_words(
                            addr, [value & 0xFFFF]
                        ),
                    ),
                ),
        ...
        if area in EXT_BIT_AREAS:
            ...
            return (
                Probe(
                    ...
                    write=cast(
                        Callable[[ToyopucClient, int], None],
                        lambda plc, value, no=no, bit_no=bit_no, addr=addr: (
                            plc.write_ext_multi(
        ...
        if area in EXT_WORD_AREAS:
            ...
            return (
                Probe(
                    ...
                    write=cast(
                        Callable[[ToyopucClient, int], None],
                        lambda plc, value, no=ext.no, addr=ext.addr: (
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="Probe downward to find the last writable address"
    )
    ...
    p.add_argument(
        "--start", default="", help="start from this address and move downward"
    )
    ...
    if args.auto_pending:
        ...
        if not stop_text:
            ...
            stop_text = (
                f"{area0}{0:0{width}X}"
                if prefix0 is None
                else f"{prefix0}-{area0}{0:0{width}X}"
            )
    ...
    with ToyopucClient(
        ...
        for probe_range in ranges:
            ...
            results.append(
                (probe_range.start_text, probe_range.stop_text, last_ok, first_error)
            )
