    ...
    try:
        with ToyopucClient(
            ...
            def multi_word_case() -> str:
                ...
                plc.send_via_relay(args.hops, build_multi_word_write(list(zip(addrs, target, strict=False))))
                readback = unpack_u16_le(plc.send_via_relay(args.hops, build_multi_word_read(addrs)).data)
                plc.send_via_relay(args.hops, build_multi_word_write(list(zip(addrs, original, strict=False))))
                if readback != target:
            ...
            def multi_byte_case() -> str:
                ...
                plc.send_via_relay(args.hops, build_multi_byte_write(list(zip(addrs, target, strict=False))))
                readback = list(plc.send_via_relay(args.hops, build_multi_byte_read(addrs)).data)
                plc.send_via_relay(args.hops, build_multi_byte_write(list(zip(addrs, original, strict=False))))
                if readback != target:
            ...
            def ext_word_case() -> str:
                ...
                readback = unpack_u16_le(plc.send_via_relay(args.hops, build_ext_word_read(dev.no, dev.addr, 1)).data)[
                    0
                ]
            ...
            def ext_multi_case() -> str:
                ...
                byte_original = plc.send_via_relay(args.hops, build_ext_byte_read(byte_dev.no, byte_dev.addr, 1)).data[
                    0
                ]
                word_original = unpack_u16_le(
                    plc.send_via_relay(args.hops, build_ext_word_read(word_dev.no, word_dev.addr, 1)).data
                )[0]
