    ...
    try:
        with ToyopucClient(
            ...
            def basic_bit_case() -> str:
                ...
                original = (
                    plc.send_via_relay(args.hops, build_bit_read(addr)).data[0] != 0
                )
                target = not original
                ...
                plc.send_via_relay(
                    args.hops, build_bit_write(addr, 1 if original else 0)
                )
                if readback != target:
            ...
            def basic_byte_case() -> str:
                ...
                original = plc.send_via_relay(args.hops, build_byte_read(addr, 1)).data[
                    0
                ]
                target = original ^ 0xFF
                plc.send_via_relay(args.hops, build_byte_write(addr, [target]))
                readback = plc.send_via_relay(args.hops, build_byte_read(addr, 1)).data[
                    0
                ]
            ...
            def multi_word_case() -> str:
                ...
                if readback != target:
                    raise ValueError(
                        f"expected={_format_u16_list(target)} got={_format_u16_list(readback)}"
                    )
            ...
            def ext_word_case() -> str:
                ...
                original = unpack_u16_le(
                    plc.send_via_relay(
                        args.hops, build_ext_word_read(dev.no, dev.addr, 1)
                    ).data
                )[0]
                target = original ^ 0xFFFF
                plc.send_via_relay(
                    args.hops, build_ext_word_write(dev.no, dev.addr, [target])
                )
                readback = unpack_u16_le(
                    plc.send_via_relay(
                        args.hops, build_ext_word_read(dev.no, dev.addr, 1)
                ...
                plc.send_via_relay(
                    args.hops, build_ext_word_write(dev.no, dev.addr, [original])
                )
                if readback != target:
                    raise ValueError(f"expected 0x{target:04X} got 0x{readback:04X}")
            ...
            def ext_byte_case() -> str:
                ...
                original = plc.send_via_relay(
                    args.hops, build_ext_byte_read(dev.no, dev.addr, 1)
                ).data[0]
                target = original ^ 0xFF
                plc.send_via_relay(
                    args.hops, build_ext_byte_write(dev.no, dev.addr, [target])
                )
                readback = plc.send_via_relay(
                    args.hops, build_ext_byte_read(dev.no, dev.addr, 1)
                ).data[0]
                plc.send_via_relay(
                    args.hops, build_ext_byte_write(dev.no, dev.addr, [original])
                )
                if readback != target:
                    raise ValueError(f"expected 0x{target:02X} got 0x{readback:02X}")
            ...
            def ext_multi_case() -> str:
                ...
                byte_original = plc.send_via_relay(
                    args.hops, build_ext_byte_read(byte_dev.no, byte_dev.addr, 1)
                ).data[0]
                word_original = unpack_u16_le(
                    plc.send_via_relay(
                        args.hops, build_ext_word_read(word_dev.no, word_dev.addr, 1)
                    ).data
                )[0]
                ...
                if (
                    bits != [bit_target]
                    or bytes_out != [byte_target]
                    or words_out != [word_target]
                ):
                    raise ValueError(
            ...
            def pc10_word_case() -> str:
                ...
                original = unpack_u16_le(
                    plc.send_via_relay(
                        args.hops, build_pc10_block_read(dev.addr32, 2)
                    ).data
                )[0]
                target = original ^ 0xFFFF
                plc.send_via_relay(
                    args.hops,
                    build_pc10_block_write(dev.addr32, target.to_bytes(2, "little")),
                )
                readback = unpack_u16_le(
                    plc.send_via_relay(
                        args.hops, build_pc10_block_read(dev.addr32, 2)
                ...
                plc.send_via_relay(
                    args.hops,
                    build_pc10_block_write(dev.addr32, original.to_bytes(2, "little")),
                )
                if readback != target:
                    raise ValueError(f"expected 0x{target:04X} got 0x{readback:04X}")
            ...
            run(
                ...
                lambda: (
                    lambda status: (
                        f"raw={status.raw_bytes_hex} run={status.run} alarm={status.alarm} pc10={status.pc10_mode}"
                    )
