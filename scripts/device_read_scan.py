...
for area, ranges in _EXT_BIT_RANGES.items():
    _add_target(
        ReadTarget(area, ranges, "ext", 4 if ranges[0][1] <= 0x1FFF else 5, "W")
    )
...
for prefix in _PREFIX_EX_NO:
    ...
    for area, ranges in _PREF_BIT_WORD_RANGES.items():
        name = f"{prefix}-{area}"
        _add_target(
            ReadTarget(name, ranges, "pref_bitword", 4, "W", prefix=prefix, area=area)
        )
...
def scan_target(
    ...
    for start, end in target.ranges:
        for chunk_start, chunk_len in _chunk_iter(start, end, chunk_size):
            ...
            print(
                f"{target.name}: {fmt(chunk_start)}-{fmt(chunk_end)} ({chunk_len} words)",
                flush=True,
            )
            try:
                if target.mode == "basic":
                    ...
                    scan_pref_word(
                        plc, prefix, area, chunk_start, chunk_len, packed=False
                    )
                elif target.mode == "pref_bitword":
                    if prefix is None:
                        raise ValueError("Prefix required for pref_bitword target")
                    scan_pref_word(
                        plc, prefix, area, chunk_start, chunk_len, packed=True
                    )
                else:
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="Read-only range scan using word-oriented access"
    )
