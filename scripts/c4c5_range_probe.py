...
def _run_bit_case(plc: ToyopucHighLevelClient, case: BitCase, log_f) -> bool:
    ...
    _print_line(
        "warning: this probe temporarily writes the selected point and restores it",
        log_f,
    )
    ...
    try:
        ...
        try:
            ...
            _print_line(
                f"phase1 alt basic read={basic_read} match_current={basic_read_ok}",
                log_f,
            )
        ...
        try:
            ...
            _print_line(
                f"restore expected={original} read={restored_read} ok={restored}", log_f
            )
        except ToyopucError as exc:
    ...
    if alias_same_point:
        ...
        return False
    _print_line(
        "verdict: basic CMD=20/21 does not alias the same point; keep CMD=C4/C5", log_f
    )
    return True
