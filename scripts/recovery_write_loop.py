...
def main() -> int:
    p = argparse.ArgumentParser(
        description="Looped write test for cable unplug/replug recovery"
    )
    p.add_argument("--host", required=True)
    p.add_argument("--port", type=int, required=True)
    p.add_argument(
        "--target", required=True, help="e.g. M0000, D0000, EX0000, U08000, P1-M0000"
    )
    ...
    p.add_argument(
        "--value", type=lambda s: int(s, 0), default=None, help="override write value"
    )
    p.add_argument(
        "--expect",
        type=lambda s: int(s, 0),
        default=None,
        help="optional expected read value",
    )
    ...
    print(
        f"value: 0x{value:X}" if not target.kind.endswith("bit") else f"value: {value}"
    )
    if args.expect is not None:
        print(
            f"expect: 0x{args.expect:X}"
            if not target.kind.endswith("bit")
            else f"expect: {args.expect & 0x01}"
        )
    ...
    try:
        ...
        _write_log(
            log_f,
            f"summary success={success_count} error={error_count} recoveries={recoveries}",
        )
