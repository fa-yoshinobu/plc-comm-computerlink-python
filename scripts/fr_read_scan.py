...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Read-only FR range scan using PC10 block read (CMD=C2)."
    )
    ...
    p.add_argument(
        "--start", type=parse_int_auto, default=0x000000, help="FR start word index"
    )
    p.add_argument(
        "--end",
        type=parse_int_auto,
        default=FR_MAX_INDEX,
    ...
    p.add_argument(
        ...
        type=parse_int_auto,
        default=0x200,
        help="words per C2 read (default: 0x200)",
    )
    p.add_argument(
        "--progress-every",
        type=int,
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="stop after this many consecutive read errors (0=disabled)",
    )
...
def main() -> int:
        ...
        with ToyopucClient(
            ...
            for index, chunk_words in iter_fr_chunks(
                ...
                try:
                    ...
                    if (
                        args.progress_every
                        and summary.ok_chunks % args.progress_every == 0
                    ):
                        elapsed = max(time.monotonic() - start_time, 1e-9)
                        progress = (
                            (summary.ok_words + summary.error_words)
                            / total_words
                            * 100.0
                        )
    ...
    print(
        f"value_words 0x0000={summary.zero_words} 0xFFFF={summary.ffff_words} other={summary.other_words}"
    )
    ...
    write_log(
        log_f, f"ok_chunks={summary.ok_chunks} error_chunks={summary.error_chunks}"
    )
