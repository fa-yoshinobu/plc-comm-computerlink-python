...
def _write_u16_to_map(
    store: MutableMapping[K, int], low_key: K, high_key: K, value: int
) -> None:
...
def handle_command(mem: Memory, frame: bytes) -> bytes:
    ...
    if cmd == 0x32:
        ...
        if sub0 == 0x71 and sub1 == 0x00:  # clock write
            ...
            try:
                mem.clock = datetime(
                    2000 + year_2digit, month, day, hour, minute, second
                )
            except ValueError:
                return build_response(cmd, b"", rc=0x01)
    ...
    if cmd == 0x94:  # ext word read
        ...
        for i in range(count):
            word_addr = addr + i
            byte_addr = (
                _program_word_byte_addr(word_addr)
                if no in (0x01, 0x02, 0x03)
                else _ext_word_byte_addr(no, word_addr)
            )
            if byte_addr is not None:
                store = (
                    mem.program_packed_byte
                    if no in (0x01, 0x02, 0x03)
                    else mem.ext_packed_byte
                )
    ...
    if cmd == 0x96:  # ext byte read
        ...
        for i in range(count):
            byte_addr = addr + i
            key_addr = (
                _program_byte_key(byte_addr)
                if no in (0x01, 0x02, 0x03)
                else _ext_byte_key(no, byte_addr)
            )
...
def run_tcp(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True:
            conn, _ = s.accept()
            t = threading.Thread(
                target=handle_tcp_client, args=(conn, mem), daemon=True
            )
            t.start()
