...
def main() -> int:
    p = argparse.ArgumentParser(description="Relay contiguous word write/read verification")
    ...
    p.add_argument("--step", type=parse_int_auto, default=1, help="per-word increment inside a block")
    p.add_argument("--loop-step", type=parse_int_auto, default=0x0100, help="per-loop increment")
    ...
    try:
        ...
        with ToyopucHighLevelClient(
            ...
            for loop_index in range(args.loops):
                expected = _pattern(args.count, args.value, args.step, loop_index, args.loop_step)
