...
def _pattern(count: int, base: int, step: int, loop_index: int, loop_step: int) -> list[int]:
...
def _relay_block_read(plc: ToyopucHighLevelClient, hops: str, device: str, count: int) -> list[int]:
...
def _relay_block_write(plc: ToyopucHighLevelClient, hops: str, device: str, values: list[int]) -> None:
    resolved = resolve_device(device)
    if resolved.unit != "word":
        raise ValueError(f"{device} is not a word device")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Relay matrix verification for larger block counts and mixed writes")
                ...
                for count in counts:
                    def _block_case(target=target, count=count) -> str:
                        ...
                        for loop_index in range(args.loops):
                            expected = _pattern(count, args.value, args.step, loop_index, args.loop_step)
            ...
            if args.clock_loops:
                def _clock_case() -> str:
                    ...
                    if passed != args.clock_loops:
                        raise ValueError(f"{passed}/{args.clock_loops} clock loops matched")
                    return (
