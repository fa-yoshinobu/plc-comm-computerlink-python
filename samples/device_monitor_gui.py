...
class MonitorApp:
    ...
    def _configure_style(self):
        ...
        style.configure(
            "Card.TLabelframe", background=self.PANEL, borderwidth=1, relief="solid"
        )
        style.configure(
    ...
    def _build_ui(self):
        ...
        ttk.Label(title_block, text="Device Monitor", style="Title.TLabel").pack(
            anchor="w"
        )
        ttk.Label(
            title_block,
        ...
        ttk.Label(connection_card, text="Endpoint", style="Meta.TLabel").grid(
            row=0, column=0, sticky="w"
        )
        ttk.Label(
            connection_card, textvariable=self.endpoint_var, style="Value.TLabel"
        ).grid(row=1, column=0, sticky="w")
        ttk.Label(connection_card, text="Connection", style="Meta.TLabel").grid(
            row=0, column=1, sticky="w", padx=(18, 0)
        )
        ttk.Label(
            connection_card, textvariable=self.connection_var, style="Value.TLabel"
        ...
        connection_controls = ttk.LabelFrame(
            sidebar, text="Connection", style="Card.TLabelframe", padding=12
        )
        connection_controls.pack(fill="x")
        connection_controls.columnconfigure(1, weight=1)

        ttk.Label(connection_controls, text="Host", style="Meta.TLabel").grid(
            row=0, column=0, sticky="w"
        )
        ttk.Entry(connection_controls, textvariable=self.host_var, width=20).grid(
            row=0, column=1, columnspan=2, sticky="ew"
        )

        ttk.Label(connection_controls, text="Port", style="Meta.TLabel").grid(
            row=1, column=0, sticky="w", pady=(10, 0)
        )
        ttk.Entry(connection_controls, textvariable=self.port_var, width=10).grid(
            row=1, column=1, sticky="ew", pady=(10, 0)
        )
        ttk.Label(connection_controls, text="Protocol", style="Meta.TLabel").grid(
        ...
        ttk.Label(connection_controls, text="Local UDP Port", style="Meta.TLabel").grid(
            row=2, column=0, sticky="w", pady=(10, 0)
        )
        ttk.Entry(connection_controls, textvariable=self.local_port_var, width=10).grid(
            row=2, column=1, sticky="ew", pady=(10, 0)
        )
        ttk.Label(connection_controls, text="Timeout", style="Meta.TLabel").grid(
            row=2, column=2, sticky="w", padx=(8, 0), pady=(10, 0)
        )
        ttk.Entry(connection_controls, textvariable=self.timeout_var, width=8).grid(
            row=2, column=3, sticky="ew", pady=(10, 0)
        )

        ttk.Label(connection_controls, text="Retries", style="Meta.TLabel").grid(
            row=3, column=0, sticky="w", pady=(10, 0)
        )
        ttk.Entry(connection_controls, textvariable=self.retries_var, width=10).grid(
            row=3, column=1, sticky="ew", pady=(10, 0)
        )
        ttk.Label(connection_controls, text="Relay hops", style="Meta.TLabel").grid(
            row=3, column=2, sticky="w", padx=(8, 0), pady=(10, 0)
        )
        ttk.Entry(connection_controls, textvariable=self.hops_var, width=18).grid(
        ...
        ttk.Button(connect_row, text="Disconnect", command=self._on_disconnect).grid(
            row=0, column=1, sticky="ew", padx=6
        )
        ttk.Button(
            connect_row, text="Refresh Label", command=self._refresh_endpoint_preview
        ).grid(row=0, column=2, sticky="ew")

        watch_card = ttk.LabelFrame(
            sidebar, text="Watch Controls", style="Card.TLabelframe", padding=12
        )
        watch_card.pack(fill="x", pady=(14, 0))
        watch_card.columnconfigure(1, weight=1)

        ttk.Label(watch_card, text="Device", style="Meta.TLabel").grid(
            row=0, column=0, sticky="w"
        )
        ...
        ttk.Button(
            watch_card, text="Add", command=self._on_add, style="Accent.TButton"
        ...
        ttk.Label(watch_card, text="Value", style="Meta.TLabel").grid(
            row=2, column=0, sticky="w", pady=(10, 0)
        )
        ttk.Entry(watch_card, textvariable=self.value_var, width=20).grid(
            row=2, column=1, sticky="ew", pady=(10, 0)
        )
        ttk.Button(watch_card, text="Write", command=self._on_write).grid(
            row=2, column=2, padx=(6, 0), pady=(10, 0)
        )

        ttk.Label(watch_card, text="Interval (s)", style="Meta.TLabel").grid(
            row=3, column=0, sticky="w", pady=(10, 0)
        )
        ttk.Entry(watch_card, textvariable=self.interval_var, width=20).grid(
            row=3, column=1, sticky="ew", pady=(10, 0)
        )
        ttk.Button(watch_card, text="Apply", command=self._on_interval).grid(
        ...
        ttk.Button(action_row, text="Read Selected", command=self._on_read).grid(
            row=0, column=0, sticky="ew"
        )
        ttk.Button(action_row, text="Remove Selected", command=self._on_remove).grid(
            row=0, column=1, sticky="ew", padx=6
        )
        ttk.Button(
            action_row,
        ...
        tools_card = ttk.LabelFrame(
            sidebar, text="PLC Actions", style="Card.TLabelframe", padding=12
        )
        tools_card.pack(fill="x", pady=(14, 0))
        for column in range(2):
            tools_card.columnconfigure(column, weight=1)
        ttk.Button(tools_card, text="Read Clock", command=self._on_clock).grid(
            row=0, column=0, sticky="ew"
        )
        ttk.Button(tools_card, text="Read Status", command=self._on_status).grid(
            row=0, column=1, sticky="ew", padx=(8, 0)
        )
        ttk.Button(tools_card, text="Clear Log", command=self._on_clear_log).grid(
            row=1, column=0, sticky="ew", pady=(8, 0)
        )

        session_card = ttk.LabelFrame(
            sidebar, text="Session", style="Card.TLabelframe", padding=12
        )
        ...
        monitor_card = ttk.LabelFrame(
            monitor_area, text="Monitor", style="Card.TLabelframe", padding=12
        )
        ...
        log_card = ttk.LabelFrame(
            monitor_area, text="Event Log", style="Card.TLabelframe", padding=12
        )
    ...
    def _session_row(
        self, parent: ttk.LabelFrame, row: int, label: str, variable: tk.StringVar
    ):
        ttk.Label(parent, text=label, style="Meta.TLabel").grid(
    ...
    def _connect(self, report_dialog: bool):
        ...
        if not self.host_var.get().strip():
            ...
            self._log(
                "connection not attempted: enter host and press Connect", level="WARN"
            )
            return
        try:
    ...
    def _expand_device_token(cls, token: str) -> list[str]:
        ...
        if start_area != end_area or start_suffix != end_suffix:
            raise ValueError(
                f"range must keep the same device family and suffix: {token}"
            )
        ...
        if end_value < start_value:
            raise ValueError(
                f"range end must be greater than or equal to start: {token}"
            )
    ...
    def _on_status(self):
        try:
            ...
            hops = self._relay_hops()
            status = (
                client.relay_read_cpu_status(hops) if hops else client.read_cpu_status()
            )
    ...
    def _poll_once(self):
        ...
        for device in self.tree.get_children(""):
            ...
            try:
                ...
                changed = (
                    device in self.last_values and self.last_values[device] != value
                )
                self.last_values[device] = value
                state = "changed" if changed else "ok"
                tags = ("changed",) if changed else ("ok",)
                self._update_tree_row(
                    device, resolved.scheme, unit, state, formatted, tags=tags
                )
            except Exception as exc:
                resolved = resolve_device(device)
                self._update_tree_row(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Tkinter device monitor example for toyopuc-computerlink"
    )
