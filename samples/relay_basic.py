...
def main() -> int:
    ...
    p.add_argument(
        "--commit", action="store_true", help="commit FR write after RAM update"
    )
    ...
    with ToyopucHighLevelClient(
        ...
        if args.mode == "clock-write":
            if args.clock_value is None:
                raise SystemExit(
                    "--mode clock-write requires --clock-value 2026-03-10T12:34:56"
                )
        ...
        if args.mode == "fr-commit":
            plc.relay_commit_fr(
                args.hops, args.device, count=args.count, wait=args.wait
            )
