...
def _parse_pc10_multi_bit_data(raw: bytes | memoryview, count: int) -> list[int]:
...
def _parse_ext_multi_bit_data(data: bytes | memoryview, count: int) -> list[int]:
    need = (count + 7) // 8
    if len(data) != need:
...
def _parse_pc10_multi_word_data(data: bytes | memoryview, count: int) -> list[int]:
    need = 4 + count * 2
    if len(data) != need:
...
def _write_pc10_block_word(client: ToyopucClient, addr32: int, value: int) -> None:
    normalized = _normalize_word_values([value])[0]
    client.pc10_block_write(addr32, normalized.to_bytes(2, "little"))
